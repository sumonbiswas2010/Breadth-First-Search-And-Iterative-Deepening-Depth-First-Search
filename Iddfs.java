 ...
 * Homework 2 - CS 411
 * File 2 of 2: Iterative Deepening Depth First Search Algorithm Implementation
 * 
...
public class Iddfs {
    ...
    public static branch IDDFS(branch firstBranch){	//Iterative Deepening Depth Search method
    	int depth = 0;
        ...
        while(depth < Integer.MAX_VALUE){
            ...
            while(!list.isEmpty()){
                ...
                branch recent = list.removeLast();
                String lastVisited = markDown(recent);

                if(goalCheck(recent)){
                    return recent;
                }
                
                else if(!marked.contains(lastVisited) && recent.tree < depth){
                    
                	marked.add(lastVisited);
                    searchBranches(recent);

                    if(recent.left != null){
                        list.add(recent.left);
                    }
                    
                    if(recent.right != null){
                        list.add(recent.right);
                    }
                    ...
                    if(recent.down != null){
                        list.add(recent.down);
                    }
                }
            }
            depth++;	// Go into the depth until the solution branch/state is not discovered 
        }
        return null;
    }
    ...
    public static void puzzleSolution(branch output, boolean display){
        ...
        System.out.println("The Iterative Deepening Depth First Search solution for the puzzle is: \n");
       ...
       for (int i =0; i < 4; i++) {
        ...
        System.out.println("Welcome to the 15-Puzzle Game!\n");
       	...
       	try{	// Try to find the solution for the puzzle, if runs out of memory, error message is displayed 
       		...
       		System.out.println("\nError: Iterative Deepening Depth First Search ran out of memory -> Cannot find the solution!\n");
       }
	}
}